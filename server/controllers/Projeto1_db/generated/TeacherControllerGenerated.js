...
const generatedControllers = {
  ...
  init: router => {
    ...
    router.get(baseUrl + "/findBycourses/:key", authorize([]), TeacherController.findBycourses);
  ...
  */
  ...
  /**
  * TeacherModel.findBycourses
  *   @description CRUD ACTION findBycourses
  *   @param Objectid key Id of model to search for
  *
  */
  findBycourses: async (req, res) => {
    try {
      const result = await TeacherModel.findBycourses(req.params.key);
      res.json(result);
    } catch (err) {
      const safeErr = ErrorManager.getSafeError(err);
      res.status(safeErr.status).json(safeErr);
